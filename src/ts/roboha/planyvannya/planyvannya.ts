...
class SchedulerApp {
  ...
  private async handleCreateWorkshop(data: PostData): Promise<void> {
    const { cehTitle, title, subtitle } = data;

    if (!cehTitle || !title || !subtitle) {
      showNotification("Будь ласка, заповніть всі поля (Цех, Пост, Прізвище)", "error");
      return;
    }
    ...
    try {
      ...
      if (existingCat) {
        ...
        // Find max ID
        const { data: maxCat, error: maxCatError } = await supabase
          .from('post_category')
          .select('category_id')
          .order('category_id', { ascending: false })
          .limit(1)
          .maybeSingle();
        if (maxCatError) throw maxCatError;

        categoryId = (maxCat?.category_id || 0) + 1;

        const { error: insertCatError } = await supabase
          .from('post_category')
          .insert({ category_id: categoryId, category: cehTitle });
      ...
      const { data: existingPost, error: postError } = await supabase
        ...
        .ilike('name', title)
        .maybeSingle();
      // Removed .eq('category', categoryId) to find if post exists globally or we should reuse it
      // If post exists with same name but different category? 
      // Logic: Post names should probably be unique or we treat them as unique entities.

      if (postError && postError.code !== 'PGRST116') throw postError;

      if (existingPost) {
        postId = existingPost.post_id;
        // Optional: Update category of existing post if it differs? 
        // For now, let's assume if it exists, we use it. 
        // But we should probably link it to the NEW category if it's not linked?
        // Let's safe-update category just in case
        await supabase.from('post_name').update({ category: categoryId }).eq('post_id', postId);
      } else {
        const { data: maxPost, error: maxPostError } = await supabase
          .from('post_name')
          .select('post_id')
          .order('post_id', { ascending: false })
          .limit(1)
          .maybeSingle();
        if (maxPostError) throw maxPostError;

        postId = (maxPost?.post_id || 0) + 1;

        const { error: insertPostError } = await supabase
          .from('post_name')
          .insert({ post_id: postId, name: title, category: categoryId });
        if (insertPostError) throw insertPostError;
      }

      // 3. Create Slyusar
      // Find max namber
      const { data: maxNamberData } = await supabase
        .from('slyusars')
        .select('namber')
        .order('namber', { ascending: false })
        .limit(1)
        .maybeSingle();

      const nextNamber = (maxNamberData?.namber || 0) + 1;

      const { error: slError } = await supabase
        .from('slyusars')
        .insert({
          data: { Name: subtitle, Опис: {}, Доступ: "Слюсар" },
          namber: nextNamber,
          post_sluysar: String(postId)
        });

      if (slError) throw slError;

      showNotification("Цех, Пост та Працівника успішно створено!", "success");
      ...
      console.error(e);
      showNotification("Помилка при створенні: " + (e.message || e), "error");
    }
  }
