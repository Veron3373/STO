...
export async function closeZakazNaraydModal(): Promise<void> {
  ...
  if (modalOverlay) {
    ...
    const actId = modalOverlay.getAttribute("data-act-id");
    if (actId) {
      const { unlockAct } = await import("./modalMain");
      await unlockAct(Number(actId));
    }
    ...
    (await import("./modalMain")).cleanupSlusarsOnSubscription();
    // 🧹 Очищуємо Realtime підписку на блокування акту
    (await import("./modalMain")).cleanupActLockSubscription();
