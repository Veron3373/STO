 ...
 */
async function highlightAddedItems(addedChanges: ChangeRecord[]): Promise<void> {
    console.log(`🔵 Підсвічування ${addedChanges.length} доданих позицій ОДНОЧАСНО...`);
...
function generateDeletedItemHtml(change: ChangeRecord, index: number): string {
    ...
    const zarplataCellHTML = showZarplata
        ? `<td class="text-right slyusar-sum-cell" data-name="slyusar_sum">${change.zarplata > 0 ? formatNum(change.zarplata) : ""}</td>`
        : "";
    ...
    return `
        <tr class="deleted-item-row">
            ...
            ${catalogCellHTML}
            <td class="text-right qty-cell" data-name="id_count">${formatNum(change.kilkist)}</td>
            <td class="text-right price-cell" data-name="price">${formatNum(change.cina)}</td>
 ...
 */
async function highlightDeletedItems(deletedChanges: ChangeRecord[]): Promise<void> {
    console.log(`🔴 Підсвічування ${deletedChanges.length} видалених позицій ОДНОЧАСНО...`);
    ...
    for (const change of deletedChanges) {
        try {
            ...
            const row = tableBody.querySelector("tr.deleted-item-row:last-child") as HTMLTableRowElement;
...
async function loadChangesForAct(actId: number): Promise<{
    ...
    console.log(`📊 Завантажено змін для акту #${actId}: додано=${added.length}, видалено=${deleted.length}`);
...
/**
 * Перевіряє та підсвічує зміни в акті для Адміністратора
 */
export async function checkAndHighlightChanges(actId: number): Promise<void> {
    // ⚠️ КРИТИЧНО: Тільки для Адміністратора
    if (userAccessLevel !== "Адміністратор") {
        console.log("⏭️ Підсвічування змін доступне тільки для Адміністратора");
        return;
    }

    try {
        console.log(`🔍 Перевірка змін для акту #${actId}...`);

        // Додаємо CSS стилі
        injectHighlightStyles();

        // Завантажуємо зміни з БД
