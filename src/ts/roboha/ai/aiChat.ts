  ...
  Для слюсаря: [FILTER:слюсар:Прізвище]. Мінімум слів.`;
    ...
    while (triedIndices.size < keys.length) {
      ...
      if (response.status === 429 || response.status === 413) {
        ...
        if (lockKey) {
          console.warn(
            `⚠️ ${provider} ключ №${keyIdx + 1}: ${reason}. Ключ зафіксовано 🔒 — ротація вимкнена.`,
          );
          return `⏳ ${reason === "запит завеликий" ? "Запит завеликий" : "Ліміт вичерпано"} для ключа №${keyIdx + 1}. Ключ зафіксовано 🔒 — спробуйте пізніше або розблокуйте ротацію.`;
        }
        console.warn(
...
function initAIChatHandlers(modal: HTMLElement): void {
  ...
  if (lockKeyCb) {
    lockKeyCb.addEventListener("change", () => {
      ...
      console.log(
        `🔒 AI: ключ ${lockKey ? "зафіксовано" : "розблоковано (ротація увімкнена)"}`,
      );
    });
  }
