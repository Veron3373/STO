...
async function resetAllTokens(): Promise<void> {
  ...
  try {
    ...
    console.log("🔄 Новий день: токени скинуто, дата → setting_id=1");
    updateKeySelect();
  } catch (err) {
    console.warn("⚠️ Не вдалося скинути токени:", err);
  }
}
...
async function checkAndResetTokensDaily(): Promise<void> {
  ...
  try {
    ...
    if (dbDateStr === todayStr) {
      ...
      console.log("✅ AI: дата актуальна, лічильники зберігаються");
    } else {
      // b) Дата в БД застаріла (або відсутня) — новий день, скидаємо
      console.log(
        `🔄 Новий день (${dbDateStr ?? "—"} → ${todayStr}): скидання лічильників`,
      );
      await resetAllTokens();
    }
  } catch (err) {
    console.warn("⚠️ checkAndResetTokensDaily:", err);
  }
}
  ...
  realtimeTokenChannel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        console.log(
          `🔄 Realtime: токен ключ №${keyIndex + 1} (setting_id=${settingId}): ${oldToken} → ${newToken}`,
        );
      },
    )
    .subscribe();
...
async function gatherSTOContext(
  ...
  try {
    ...
    console.warn("⚠️ Помилка завантаження lookup клієнтів/авто:", err);
  }
  ...
  try {
    ...
    if (analysis.needsActs) {
      ...
      try {
        // 🔗 Якщо "за весь період" + Важкий — не фільтруємо за датою
        ...
        if (isAllTime) {
          console.log(
            `📊 AI: завантажено ${closedMonthActs.length} закритих актів за ВЕСЬ ПЕРІОД`,
          );
        }
      } catch {
        /* fallback */
    ...
    if (
      ...
      try {
        ...
        if (!analysis.searchBrand && !analysis.searchName) {
          ...
          // 🏋️ Важкий: повний список АВТО з деталями
          if (isHeavy) {
  ...
  // ============================================================
  // 13. АНАЛІТИЧНІ ПІДКАЗКИ (Важкий режим)
  // ============================================================
