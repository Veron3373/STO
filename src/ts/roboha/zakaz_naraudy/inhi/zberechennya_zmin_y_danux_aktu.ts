  ...
  try {
    ...
    try {
      ...
      if (added.length > 0 || deleted.length > 0) {
        await logAction("update_act_items", actId, {
          added: added.map((i) => ({ name: i.name, type: i.type })),
          deleted: deleted.map((i) => ({ name: i.name, type: i.type })),
        });
      } else {
        await logAction("update_act_info", actId);
      }
    } catch (logError) {
