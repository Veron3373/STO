...
- slyusars: slyusar_id, Name, data (JSONB): Name, "Ім'я", Доступ, Телефон, Посада, Пароль
- sclad: sclad_id, data (JSONB): Назва, Каталог, Виробник, Ціна, Кількість, Група, Постачальник
- act_changes_notifications: act_id, title, message, is_read, ... (та інші поля)
...
ТАБЛИЦІ (table):
- acts — акти (замовлення-наряди)
- clients — клієнти
- cars — автомобілі
- slyusars — слюсарі/персонал
- sclad — склад деталей
- act_changes_notifications — сповіщення про зміни актів

ПОЛЯ ТАБЛИЦІ acts (для check):
- date_off IS NOT NULL → акт закрився (отримав дату закриття)
- date_off IS NULL → акт відкрився (дата закриття стала null)
- date_off CLOSED → акт саме зараз закрили (date_off змінився з null на значення)
- date_off OPENED → акт саме зараз відкрили (date_off змінився з значення на null)
- date_off CHANGED → дата закриття змінилась
...
ФОРМАТ ВІДПОВІДІ (тільки JSON, без пояснень):
{
  ...
  "check": "умова_перевірки_або_порожнє"
}
...
async function generateRealtimeRuleFromDescription(description: string): Promise<string> {
  const keys = await loadAIKeys();
  if (keys.length === 0) throw new Error("Немає AI ключів");
...
function initModalHandlers(
  ...
  overlay
    ...
    ?.addEventListener("click", async () => {
      ...
      const condFreqNow = (overlay.querySelector("#planner-cond-freq") as HTMLSelectElement)?.value;
  ...
  overlay
    ...
    ?.addEventListener("click", async () => {
      ...
      if (selectedType === "once") {
        ...
        if (condDesc && !condQuery) {
          const condFreqEarly = (overlay.querySelector("#planner-cond-freq") as HTMLSelectElement).value;
          try {
