//src\ts\roboha\dodatu_inchi_bazu\inhi\pruimalnuk.ts
  ...
  if (currentLoadedData && currentLoadedData.length > 0) {
    for (const item of currentLoadedData) {
      ...
      if (valueToCheck?.trim() === inputValue.trim()) {
        ...
        // 🔥 SAVE ID for editing
        if (idValue) {
          localStorage.setItem("current_receiver_id", String(idValue));
        }
...
export const handlePruimalnikClick = async () => {
  await loadDatabaseData("Приймальник");
};
