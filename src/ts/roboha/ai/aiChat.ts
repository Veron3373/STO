...
▸ Наявність запчастин — шукай в інтернет-магазинах (autodoc, exist, emex тощо)
▸ Будь-що інше що потребує актуальної інформації з інтернету

📌 ОБОВ'ЯЗКОВО:
1. Активно використовуй пошук для будь-яких запитів що потребують зовнішньої інформації
2. Надавай ПОСИЛАННЯ на джерела (магазини, статті, каталоги)
3. Порівнюй ціни з різних джерел коли можливо
4. Вказуй актуальність знайденої інформації
    ...
    while (triedIndices.size < keys.length) {
      ...
      if (response.ok) {
        ...
        if (provider === "groq") {
          ...
          // 💡 Google Search Grounding може повертати кілька parts —
          // збираємо текст зі ВСІХ parts, а не лише parts[0]
          const candidate = data?.candidates?.[0];
          const parts = candidate?.content?.parts;
          if (Array.isArray(parts) && parts.length > 0) {
            // Збираємо текст з усіх частин (пропускаємо functionCall/functionResponse)
            const textParts = parts.map((p: any) => p.text).filter(Boolean);
            if (textParts.length > 0) {
              text = textParts.join("\n");
            }
            // Якщо текст порожній, перевіряємо чи є functionCall
            // (Google Search може повернути executableCode або інші типи parts)
            if (!text) {
              const otherContent = parts
                .map((p: any) => {
                  ...
                  if (p.codeExecutionResult)
                    return p.codeExecutionResult.output;
                  return null;
                })
                .filter(Boolean);
          ...
          // Також перевіряємо searchEntryPoint для пошукових посилань
          if (grounding?.searchEntryPoint?.renderedContent && text) {
            // renderedContent — HTML віджет пошуку, не додаємо його
          }
