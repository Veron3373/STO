...
- date_off CHANGED → будь-яка зміна поля (включаючи очищення в null і встановлення з null)
- date_off IS NOT NULL → поле не порожнє
- date_off IS NULL → поле порожнє

ВАЖЛИВО: "CHANGED" спрацьовує на БУДЬ-ЯКУ зміну поля:
  - значення → інше значення
  - значення → null (очищення комірки)
  - null → значення (заповнення комірки)
Якщо потрібно відстежити лише очищення — використовуй OPENED.
Якщо потрібно відстежити лише заповнення — використовуй CLOSED.
