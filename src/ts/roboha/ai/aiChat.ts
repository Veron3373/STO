...
/**
 * Завантажує накопичену кількість токенів для ключа з БД (стовпець token)
 */
async function loadTokensFromDB(settingId: number): Promise<number> {
  if (settingId <= 0) return 0;
  try {
    const { data } = await supabase
      .from("settings")
      .select("token")
...
/**
 * Додає токени до накопиченого значення в БД для конкретного ключа
 */
async function saveTokensToDB(
  ...
  try {
    const current = await loadTokensFromDB(settingId);
    const newTotal = current + tokensToAdd;
    await supabase
      .from("settings")
      .update({ token: newTotal })
      .eq("setting_id", settingId);
    console.log(
      `🎫 Токени: ключ №${settingId} → ${newTotal.toLocaleString("uk-UA")} (+${tokensToAdd.toLocaleString("uk-UA")})`,
    );
  } catch (err) {
    console.warn("⚠️ Не вдалося зберегти токени в БД:", err);
  }
}
...
async function resetAllTokens(): Promise<void> {
  try {
    ...
    console.log("🔄 Токени скинуто для всіх ключів");
    localStorage.setItem("aiTokensLastReset", new Date().toISOString());
    await updateKeySelect(); // Оновлюємо UI
  } catch (err) {
    console.warn("⚠️ Не вдалося скинути всі токени:", err);
  }
}
 ...
 */
async function updateKeySelect(): Promise<void> {
  ...
  // Завантажуємо токени для всіх ключів паралельно
  const tokenPromises = geminiKeySettingIds.map((sid) =>
    sid > 0 ? loadTokensFromDB(sid) : Promise.resolve(0),
  );
  ...
  // Оновлюємо лічильник токенів для поточного ключа
  const settingId = geminiKeySettingIds[currentKeyIndex];
  if (settingId > 0) {
    const totalTokens = await loadTokensFromDB(settingId);
    updateTokenCounter(0, totalTokens);
  }
}
...
async function gatherSTOContext(
  ...
  if (isHeavy) {
    // 🏋️ ВАЖКИЙ — ВСЕ підвантажується БЕЗ ВИКЛЮЧЕНЬ
  ...
  try {
    ...
    if (analysis.needsActs) {
      ...
      // 💡 Ліміти залежать від рівня (Важкий — без обмежень, максимальний доступ)
    ...
    if (slyusarsData.length > 0) {
      ...
      slyusarsData.forEach((s: any) => {
        ...
        if (isHeavy) {
          // 🏋️ ВАЖКИЙ: повна інформація по кожному слюсарю
          ...
          if (d["Історія"]) {
            ...
            for (const [date, records] of Object.entries(d["Історія"])) {
              if (date >= monthStart) {
                ...
                arr.forEach((rec: any) => {
                  ...
                  monthEntries.push(
                    `    📅 ${date} | Акт №${rec["Акт"] || "?"} | Роботи: ${sumaRoboty} грн | ЗП роботи: ${zpRoboty} грн | ЗП запч: ${zpZapch} грн | Разом ЗП: ${zpRoboty + zpZapch} грн`,
                  );
                });
              }
            }
            ...
            if (monthEntries.length > 0) {
              context += `    📊 За місяць: ${monthActsCount} актів, ЗП разом: ${monthSalary.toLocaleString("uk-UA")} грн\n`;
    ...
    // ============================================================
    // 2.1 ВАЖКИЙ: Зв'язок Слюсар ↔ Акти (повна картина)
    // ============================================================
    if (
      isHeavy &&
    ...
    if (
      ...
      // 💡 Обмежуємо: ліміти залежать від рівня (Важкий — максимальний доступ)
  ...
  Для слюсаря: [FILTER:слюсар:Прізвище]. Мінімум слів.`;
    ...
    while (triedIndices.size < keys.length) {
      ...
      if (response.ok) {
        ...
        // 💾 Зберігаємо токени в БД для поточного ключа
        const settingId = geminiKeySettingIds[keyIdx];
        if (settingId > 0) {
          saveTokensToDB(settingId, usageTokens).then(async () => {
            const total = await loadTokensFromDB(settingId);
...
export async function createAIChatModal(): Promise<void> {
    ...
    <div class="ai-chat-window">
      ...
      <div class="ai-chat-panel" id="ai-panel-chat">
        ...
        <div class="ai-chat-statusbar">
          ...
          <span id="ai-token-counter" class="ai-token-counter" title="Токени використані сьогодні. Скидається автоматично щодня.">🎫 —</span>
          <button id="ai-reset-tokens-btn" class="ai-reset-tokens-btn" title="Скинути лічильник токенів (вручну)" type="button">🔄</button>
        </div>
      </div>
  ...
  // + перевіряємо чи не треба скинути токени автоматично
...
function initAIChatHandlers(modal: HTMLElement): void {
  ...
  // ── Кнопка скидання токенів ──
  const resetTokensBtn = modal.querySelector("#ai-reset-tokens-btn");
  if (resetTokensBtn) {
    resetTokensBtn.addEventListener("click", async () => {
      const lastReset = localStorage.getItem("aiTokensLastReset");
      const lastResetText = lastReset
        ? new Date(lastReset).toLocaleString("uk-UA")
        : "ніколи";

      if (
        !confirm(
          `Скинути лічильник токенів для ВСІХ ключів?\n\n⚠️ Рекомендується робити це щодня (ліміти API обнуляються кожні 24 год).\n\nОстаннє скидання: ${lastResetText}`,
        )
      )
        return;

      await resetAllTokens();
      alert("✅ Токени скинуто для всіх ключів!");
    });
  }
